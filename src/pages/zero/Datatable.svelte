<script>
  ...
  const handleDatas = (limitdata) => {
    ...
    if (search !== '') {
      ...
      if (pagging < 1) {
        ...
        datas = nobj.filter(
          (f, idxf) =>
            idxf < limitdata * (pagging + 1) && idxf >= limitdata * pagging
        );
  ...
  function handleSortBy(coloumn, sort = 'asc') {
    return function innerSort(a, b) {
      ...
      const aa =
        typeof a[coloumn] === 'string' ? a[coloumn].toUpperCase() : a[coloumn];
      const bb =
        typeof b[coloumn] === 'string' ? b[coloumn].toUpperCase() : b[coloumn];
...
<div
        ...
        <tr>
          ...
          {#each config.coloumns as coloumn}
            <th>
              <a
                ...
                on:click|preventDefault={() => handleSort(coloumn.toLowerCase(), (sortby = !sortby))}>{coloumn.replace('_', ' ')}</a>
            </th>
          {:else}
          ...
          <th data-sortable="" style="width: 20.1694%;">
            <a
              href="/dtable"
              class="dataTable-sorter"
              on:click|preventDefault={() => handleSort(false)}>Action</a>
          </th>
        </tr>
      </thead>
      <tbody>
  ...
  <div class="dataTable-bottom">
    <div class="dataTable-info">
      Showing
      {pagging + 1 > 1 ? pagging * limit + 1 : 1}
      to
      {pagging + 1 > 1 ? ((pagging + 1) * limit > rows.length ? rows.length : (pagging + 1) * limit) : limit}
      of
      {rows.length}
      entries
    </div>
    ...
    <div class="dataTable-pagination">
      <li class="pager">
        <a
          href="/dtable"
          data-page={pagging - 1}
          on:click|preventDefault={() => handlePagging(pagging - 1)}>‹</a>
      </li>

      {#each Array(paggings) as _, i}
        <li class="" class:active={i === pagging}>
          <a
            href="/dtable"
            data-page={pagging}
            on:click|preventDefault={() => handlePagging(i)}>{i + 1}</a>
        </li>
      {/each}
